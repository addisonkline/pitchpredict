...
def get_pitches_from_pitcher(pitcher_id: int) -> pd.DataFrame:
    """
    ...
    date_start = config.get('start_date')
    date_end = config.get('end_date')
...
def get_most_similar_pitches(pitches: pd.DataFrame, this_pitch: Pitch) -> pd.DataFrame:
    """
    ...
    pitch_limit = int(config.get('pitch_sample_pctg') * pitches.__len__())
